...
const { sendCatelogue } = require('./Model/sendCatelogue');
...
io.on('connection', (socket) => {
  ...
  socket.on('sendCatelogue', async(data, callback) => {
    const newSendCatelogue = new sendCatelogue(data);
    const result = await newSendCatelogue.save();
    ...
    if (result) {
      ...
      if (online['admin']) {
        socket.to(online.admin).emit('catelogue', {
          data: newSendCatelogue,
        });
      } else {
        console.log('Admin not online');
      }
